...
public static class AdminSyncing
{
    ...
    private static void AdminStatusSync(ZNet __instance)
    {
        ...
        ZRoutedRpc.instance.Register<ZPackage>(HideMePlugin.ModName + " AdminStatusSync", RPC_AdminAbilityCheck);
    ...
    private static IEnumerator<bool> TellPeerAdminStatus(ZNetPeer peer, ZPackage package)
    {
        ...
        {
            const string method = HideMePlugin.ModName + " AdminStatusSync";
            if (_isServer)
            {
    ...
    internal static void RPC_AdminAbilityCheck(long sender, ZPackage package)
    {
        ...
        if (_isServer)
        {
            ...
            if (ZNet.instance.ListContainsId(ZNet.instance.m_adminList, currentPeer.m_rpc.GetSocket().GetHostName()))
            {
                ZPackage pkg = new();
                pkg.Write(true);
                currentPeer.m_rpc.Invoke(HideMePlugin.ModName + " AdminStatusSync", pkg);
                HideMePlugin.HideMeLogger.LogDebug($"Admin status for {currentPeer.m_playerName} is: {admin}");
            }
        }
        else
        {
            HideMePlugin.HideMeLogger.LogInfo($"Admin status: {admin}");
            HideMePlugin.isAdmin = admin;
        }
    }
    ...
    class RegisterClientRPCPatch
    {
        private static void Postfix(ZNet __instance, ZNetPeer peer)
        {
            ...
            {
                peer.m_rpc.Register<ZPackage>(HideMePlugin.ModName + " AdminStatusSync",
    ...
    static class ZNet_Awake_Patch
    {
        static void Postfix(ZNet __instance)
        {
            ...
            {
                HideMePlugin.isAdmin = true;
                HideMePlugin.HideMeLogger.LogDebug($"Local Play Detected setting Admin: {HideMePlugin.isAdmin}");
            }
        }
    }
}